...
public:
    ...
    */
    ...
    // 입력 Mat, 채널 번호
    std::vector<TrackingResult>detect_and_track(const cv::Mat& frame,int cam_ch, float detect_thr);
    std::vector<DetectResult>MOTWrapper::detect(const cv::Mat& frame);
    void apply_ptz_motion(int cam_ch, float hfov, float d_pan, float d_tilt);
    std::vector<TrackingResult> get_active_tracks(int cam_ch);
...
private:
    // DLL handles
    void* m_yolo;
    std::map<std::string, void*> m_detectors;
...
private:
    // 내부 유틸
    LetterboxInfo apply_letterbox_tl(const cv::Mat& src, cv::Mat& dst, int target_size);
    LetterboxInfo apply_letterbox(const cv::Mat& src, cv::Mat& dst, int target_size);
    LetterboxInfo lb = { 0.0f, 0, 0 };
};